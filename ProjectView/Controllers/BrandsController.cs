...
namespace ProjectViews.Controllers
{
    public class BrandsController : Controller
    {
        ...
        {
            string apiUrl = $"https://localhost:7110/api/Brands/get-all-brands";
        ...
        {
            string apiUrl = $"https://localhost:7110/api/Brands/create-brand?brandName={brands.Name}";
        ...
        {
            string apiUrl = $"https://localhost:7110/api/Brands/get-brand-by-id?id={id}";
            var response = await _httpClient.GetAsync(apiUrl);
            string apidata = await response.Content.ReadAsStringAsync();
        ...
        {
            string apiUrl =
                $"https://localhost:7110/api/Brands/update-brand-by-id?id={brands.Id}&brandName={brands.Name}";
            var content = new StringContent(JsonConvert.SerializeObject(brands), Encoding.UTF8, "application/json");
            var response = await _httpClient.PostAsync(apiUrl, content);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            string apiUrl = $"https://localhost:7110/api/Brands/delete-brands-by-id?id={id}";
        ...
        public async Task<IActionResult> DeleteManyBrands(List<Guid> id)
        {
            try
            {
                if (id != null && id.Count > 0)
                {
                    ...
                    {
                        var url = "https://localhost:7110/api/Brands/delete-many-brands";
