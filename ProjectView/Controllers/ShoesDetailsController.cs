...
namespace ProjectViews.Controllers
{
    public class ShoesDetailsController : Controller
    {
            ...
            else
            {
                for (int i = 0; i < sizesList.Count(); i++)
                {
                    for (int j = 0; j < colorsList.Count(); j++)
                    {
                        ...
                        string urlApi =
                            $"https://localhost:7110/api/ShoeDetails/create-shoeDetails?IDShoeDetails={newshoes.Id}&name={newshoes.Name}&costPrice={newshoes.CostPrice}&sellPrice={newshoes.SellPrice}&availableQuantity={newshoes.AvailableQuantity}&status={newshoes.Status}&idSupplier={newshoes.IdSupplier}&idCategory={newshoes.IdCategory}&idBrand={newshoes.IdBrand}&idSale={newshoes.IdSale}";
                        var response = await _httpClient.PostAsync(urlApi, null);
                        string urlApiSize =
                            $"https://localhost:7110/api/SIzes_ShoeDetails/create-size-shoe-details?sizeId={sizesList[i]}&shoeDetailsId={newshoes.Id}";
                        var responseSize = await _httpClient.PostAsync(urlApiSize, null);
                        string urlApiColor =
                            $"https://localhost:7110/api/Color_ShoeDetails/create-color-shoeDetails?idShoeDetails={newshoes.Id}&idColor={colorsList[j]}";
        ...
        {
            var response =
                await _httpClient.GetAsync($"https://localhost:7110/api/ShoeDetails/get-shoeDetails-by-id?id={id}");
        ...
        public async Task<IActionResult> Edit(ShoeDetails shoeDetails, Guid idSize, Guid idColor)
        {
            ...
            //         $"https://localhost:7110/api/ShoeDetails/get-shoeDetails-by-name?name={shoeDetails.Name}");
            ...
            //         await _httpClient.GetAsync(
            //             $"https://localhost:7110/api/Color_ShoeDetails/get-color-shoesDetails-by-id?id={idColor}");
            ...
            //         await _httpClient.GetAsync(
            //             $"https://localhost:7110/api/SIzes_ShoeDetails/get-size-shoe-details-by-id?id={idSize}");
            ...
            // string urlApiSize =
            //     $"https://localhost:7110/api/SIzes_ShoeDetails/update-size-shoe-details?sizeId={idSize}&shoeDetailsId={shoeDetails.Id}";
            // var responseSize = await _httpClient.PutAsync(urlApiSize, null);
            // //update color_shoeDetails
            // string urlApiColor =
            //     $"https://localhost:7110/api/Color_ShoeDetails/update-color-shoesDetails?idShoeDetails={shoeDetails.Id}&idColor={idColor}";
        ...
        {
            string apiUrl = $"https://localhost:7110/api/ShoeDetails/delete-shoedetails?id={id}";
        ...
        public async Task<IActionResult> DeleteMany(List<Guid> deleteMany)
        {
            ...
            {
                string apiUrl = $"https://localhost:7110/api/ShoeDetails/delete-shoedetails?id={item}";
                var response = await _httpClient.DeleteAsync(apiUrl);
            }
