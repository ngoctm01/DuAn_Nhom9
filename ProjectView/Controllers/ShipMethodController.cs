...
namespace ProjectViews.Controllers
{
    public class ShipMethodController : Controller
    {
        ...
        {
            string apiURL = $"https://localhost:7110/api/ShipMethod";
        ...
        {
            string apiURL = $"https://localhost:7110/api/ShipMethod/{Id}";
            var response = await _httpClient.GetAsync(apiURL);
            var apiData = await response.Content.ReadAsStringAsync();
        ...
        {
            string apiURL = $"https://localhost:7110/api/ShipMethod/CreateShipMethod?Name={shipAdressMethod.NameAddress}&status={shipAdressMethod.Status}&price={shipAdressMethod.Price}";
        ...
        {
            string apiURL = $"https://localhost:7110/api/ShipMethod/UpdateShipMethod?Id={Id}&Name={shipAdressMethod.NameAddress}&status={shipAdressMethod.Status}&price={shipAdressMethod.Price}";
            var content = new StringContent(JsonConvert.SerializeObject(shipAdressMethod), Encoding.UTF8, "application/json");
        ...
        {
            string apiURL = $"https://localhost:7110/api/ShipMethod/DeleteShipMethod?Id={Id}";
