...
namespace ProjectViews.Controllers
{
    public class SalesController : Controller
    {
        ...
        {
            string apiURL = $"https://localhost:7110/api/Sales/Show-Sales";
        ...
        {
            string apiURL = $"https://localhost:7110/api/Sales/{Id}";
            var response = await _httpClient.GetAsync(apiURL);
            var apiData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Create(Sales sales)
        {
            ...
            string apiURL = $"https://localhost:7110/api/Sales/Create-Sales?DiscountValue={sales.DiscountValue}&SaleName={sales.SaleName}";
        ...
        public async Task<IActionResult> Edit(Guid id, Sales sales)
        {
            ...
            string apiURL = $"https://localhost:7110/api/Sales/EditSales?id={id}&DiscountValue={sales.DiscountValue}&SaleName={sales.SaleName}&EndDate={formattedEndDate}";
            var content = new StringContent(JsonConvert.SerializeObject(sales), Encoding.UTF8, "application/json");
        ...
        {
            string apiURL = $"https://localhost:7110/api/Sales/Delete-Sales-/{Id}";
