...
namespace ProjectViews.Controllers
{
    public class BillController : Controller
    {
        ...
        public async Task<IActionResult> Show(int? page, int? pageSize)
        {
            ...
            string apiURL = $"https://localhost:7110/api/Bill";
        ...
        {
            string apiUrl = "https://localhost:7110/api/Bill/monthly-sales";
            var response = await _httpClient.GetAsync(apiUrl);

            if (response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
                var statistics = JsonConvert.DeserializeObject<MonthlySalesViewModel>(content);

                return View(statistics);
            }

            return View(new MonthlySalesViewModel());
        }
        // GET: BillController/Details/5
        [HttpGet]
        public async Task<IActionResult> Details(Guid Id)
        {
            string apiURLGetBill = $"https://localhost:7110/api/Bill/{Id}";
            string apiURLGetBillDetail = $"https://localhost:7110/api/BillDetails/FillterByID/{Id}";
            ...
            // Get coupon
            var apiURLCoupon = $"https://localhost:7110/api/Coupons/{bill.IdCoupon}";
            ...
            // Shoe detail
            string apiURLShoes = $"https://localhost:7110/api/ShoeDetails/get-all-shoeDetails";
        ...
        public async Task<IActionResult> Create(Bills bill)
        {
            ...
            string apiURL = $"https://localhost:7110/api/Bill/CreateBill?IdUser={bill.IdUser}&Note={bill.Note}&status={bill.Status}&IdCoupon={bill.IdCoupon}&IdShipMethod={bill.IdShipAdressMethod}&IdLocation={bill.IdLocation}&IdPaymentMethod={bill.IdPaymentMethod}";
        ...
        {
            string apiURL = $"https://localhost:7110/api/Bill/{Id}";
            var response = await _httpClient.GetAsync(apiURL);
            string apiData = await response.Content.ReadAsStringAsync();
        ...
        {
            string apiURL = $"https://localhost:7110/api/Bill/UpdateBill?id={Id}&IdUser={bill.IdUser}&Note={bill.Note}&status={bill.Status}&IdCoupon={bill.IdCoupon}&IdShipMethod={bill.IdShipAdressMethod}&IdLocation={bill.IdLocation}&IdPaymentMethod={bill.IdPaymentMethod}";
            var content = new StringContent(JsonConvert.SerializeObject(bill), Encoding.UTF8, "application/json");
